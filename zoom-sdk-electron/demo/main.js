...
function sdkauthCB(status) {
  if (ZoomAuthResult.AUTHRET_SUCCESS == status){
    ...
    zoomsetgeneral.Setting_EnableAutoFullScreenVideoWhenJoinMeeting({
      zn_bEnable: true
    })
  ...
  switch (status)
  {
    ...
    case ZoomMeetingStatus.MEETING_STATUS_ENDED:
      ...
      if (hasLogin) {
        ...
        // f().then(BrowserWindow.getFocusedWindow().minimize()); // 1
      }
      break;
    default:
    break;
...
function showDomainwindow(){
  if (!domainWindow)
  {
    ...
    domainWindow.loadURL('file://' + __dirname + '/pages/qrcode.html');
  }
  if (mainWindow) {
    mainWindow.close();
    mainWindow = null;
...
let functionObj = {
  showSatrtJoinUnLoginWindow: function() {
    if (!startjoinUnLoginWindow) {
      ...
      startjoinUnLoginWindow.loadURL('file://' + __dirname + '/pages/qrcode.html'); 
      startjoinUnLoginWindow.setFullScreen(true)
      startjoinUnLoginWindow.setMenu(null)
    }
    if (startjoinWindow)
    {
      startjoinWindow.close();
...
console.log(command)
if(command == 'enterFullscreen\r\n') zoomuicontroller.MeetingUI_EnterFullScreen({bFirstView: '1',bSecView: '0'});
else if(command == 'muteVideo\r\n') zoomvideo.MeetingVideo_MuteVideo({userid: myId});
else if(command == 'unmuteVideo\r\n') zoomvideo.MeetingVideo_UnMuteVideo({userid: myId});
else if(command == 'switchToVideoWall\r\n') zoomuicontroller.MeetingUI_SwitchToVideoWall()
else if(command == 'swtichToAcitveSpeaker\r\n') zoomuicontroller.MeetingUI_SwtichToAcitveSpeaker()
else if(command == 'muteAudio\r\n') zoomaudio.MeetingAudio_MuteAudio({userid: myId,allowunmutebyself: false})
else if (command == 'unmuteAudio\r\n') zoomaudio.MeetingAudio_UnMuteAudio({userid: myId}) 
else if(command == 'leave\r\n') zoommeeting.LeaveMeeting({endMeeting: false})
else zoommeeting.HandleZoomWebUriProtocolAction(command)
...
let checkInit = zoomsdk.InitSDK();
if (ZoomSDKError.SDKERR_SUCCESS == checkInit){
  let options={
    authcb: sdkauthCB, 
    logincb: loginretCB,
    logoutcb: null
